      ...
      <template v-for="goods in goodsList" :key="goods.id">
        <GoodsInfo :goods="goods">
          <div class="item-control">
            <van-button plain hairline round  type="primary" size="mini" @click="catNum(goods)">再次购买</van-button>
            <h4>{{goods.num}}</h4>
            <van-button plain hairline round  type="primary" size="mini" @click="addNum(goods)">取消订单</van-button>
          </div>
        </GoodsInfo>
      </template>
    </ul>
